...
select countrycode, city, population from city
group by coutrycode
order by poplulation desc
limit 5;
...
select countrycode, sum(population) from country;

-- 문제 6: Country 테이블에서 GDP(국내 총생산, GNP * Population)가 가장 높은 상위 5개 국가를 조회하세요.
select countrycode, gnp * population as '국내총생산' from country
order by 국내총생산 desc
limit 5;
